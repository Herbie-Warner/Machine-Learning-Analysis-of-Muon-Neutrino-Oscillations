...
class CNN():
    ...
    def create_convolution_1d_2d(self):
        ...
        path1 = layers.Conv1D(256, 2, activation='relu')(input_tensor)
        #path1 = layers.Dropout(dropoutrate)(path1)
        path1 = layers.MaxPooling1D(2)(path1)

        path1 = layers.Conv1D(128, 2, activation='relu')(path1)
        path1 = layers.GlobalMaxPooling1D()(path1)
  

 
        path2 = layers.Conv2D(256, (2, 2), activation='relu')(input_tensor_2d)
        #path2 = layers.Dropout(dropoutrate)(path2)
        path2 = layers.MaxPooling2D((2, 2))(path2)
 
        path2 = layers.Conv2D(128, (2, 2), activation='relu')(path2)
        path2 = layers.GlobalMaxPooling2D()(path2)
    
        

        path3 = layers.Dense(256, activation='relu')(input_tensor)
       # path3 = layers.Dropout(dropoutrate)(path3)
        path3  =  layers.Dense(256, activation='relu')(path3)
        path3  =  layers.Dense(256, activation='relu')(path3)
       # path3 = layers.Dropout(dropoutrate)(path3)
        path3  =  layers.Dense(128, activation='relu')(path3)
        path3 = layers.Flatten()(path3)
        
        path4 = layers.Flatten()(input_tensor)
        path4 = layers.Dense(512,activation='relu')(path4)
        path4 = layers.Dense(512,activation='relu')(path4)
        path4 = layers.Dense(128,activation='relu')(path4)
        path4 = layers.Dense(128,activation='relu')(path4)
        ...
        dense_layer1 = layers.Dense(64, activation='relu')(dense_layer)
        dense_layer2 = layers.Dense(64, activation='relu')(dense_layer)
        dense_layer3 = layers.Dense(64, activation='relu')(dense_layer)
           
       
        
        output1 = layers.Dense(1, activation='tanh')(dense_layer1)  # Output layer for binary classification
        output2 = layers.Dense(1, activation='sigmoid')(dense_layer2)  # Output layer for binary classification
        output3 = layers.Dense(1, activation='sigmoid')(dense_layer3)  # Output layer for binary classification
    ...
    def other_train_model(self,x,Y):
        ...
        plt.title(r'$\chi^2$ Error Rate Against Epoch For Hybrid CNN',fontsize=16, fontname='Times New Roman')
        ...
        plt.ylabel(r'$\chi^2$ Error',fontsize=12, fontname='Times New Roman')
        plt.legend(loc='upper right')
        plt.grid(True)
        #plt.savefig('error_rate_against_epoch_CNN_safe.png',dpi=600)
